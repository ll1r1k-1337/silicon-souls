...
- UI для настройки LLM provider;
- проверку подключения к provider;
- отображение активной модели и маскированного API key.
...
- вызов LLM provider;
- чтение и сохранение LLM provider settings;
- шифрование и маскирование API keys;
...
Настройки provider являются пользовательскими настройками приложения и управляются через UI. Backend API при каждом LLM вызове выбирает provider из сохранённых настроек, а не требует ручного изменения `.env`.

Для production `.env` может содержать только bootstrap/fallback значения и ключ шифрования настроек. Основной путь настройки:

```text
Settings UI -> API /settings/llm -> PostgreSQL encrypted settings -> LlmProvider resolver
```
